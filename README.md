...
```
### Add user to tomcat10 to access Manager GUI Pannel
```console
...
[INFO] Tests run: 5, Failures: 0, Errors: 0, Skipped: 0
```
### Manual deploy the app (.war file) to tomcat 
#### Deploy the calculator.war manually for now through Manager GUI 
1. Access the Tomcat Manage GUI: <tomcat_server_public_ip_address>:8080/manager
2. User the username and password which we configure in tomcat server inside <tomcat-users> ..... </tomcat-users>
